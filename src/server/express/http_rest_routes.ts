...
/**
 * Creates an Express router with HTTP+REST routes for the A2A Protocol
 * 
 * This router implements the A2A REST API specification, providing
 ...
 * 
 * @param requestHandler - The A2A request handler implementation that contains business logic
 * @returns An Express router configured with all A2A REST endpoints
 * 
 * @example
 * ```typescript
 * const requestHandler = new DefaultRequestHandler(executor, store);
 * const router = createHttpRestRouter(requestHandler);
 * app.use(router);
 * ```
 */
export function createHttpRestRouter(
