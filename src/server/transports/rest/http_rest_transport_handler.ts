...
export class RestTransportHandler {
  ...
  private normalizePart(part: PartInput): Part {
    ...
    if (part.kind === 'file') {
      const file = this.normalizeFile(part.file as FileInput);
  ...
  private normalizeMessageSendParams(input: MessageSendParamsInput): MessageSendParams {
    ...
    const config = p.configuration as
      ...
      | undefined;
    const pnConfig = config?.push_notification_config ?? config?.pushNotificationConfig;
            ...
            pushNotificationConfig: pnConfig
              ? {
                  authentication: pnConfig.authentication
                    ? {
                        schemes: pnConfig.authentication.schemes,
                        credentials: pnConfig.authentication.credentials,
                      }
                    : undefined,
                  id: pnConfig.id,
                  url: pnConfig.url,
