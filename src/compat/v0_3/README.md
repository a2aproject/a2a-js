...
| `@a2a-js/sdk/compat/v0_3/translate`      | The bidirectional v0.3 ↔ v1.0 payload translators (`toCore<Entity>`, `toCompat<Entity>`) documented below, plus the v0.3 wire types under the `legacy` namespace. Opt-in: kept off the main `compat/v0_3` entry so importing it doesn't pull the translation layer into a Workers bundle that only needs the transports.                                                                                 | none (Workers-safe) |

The translators were unpublished until `compat/v0_3/translate` opened them up in response to [#646](https://github.com/a2aproject/a2a-js/issues/646): a receiver that isn't an SDK client has a raw v0.3 body on its webhook route and no other way to lift it to v1.0. They may still change without a major-version bump.
