...
describe('DefaultRequestHandler as A2ARequestHandler', () => {
  ...
  it('sendMessage: should return a simple message response', async () => {
    ...
    assert.deepEqual(
      result,
      agentResponse,
      "The result should be the agent's message",
    );
    assert.isTrue(
  ...
  it('sendMessage: (non-blocking) should return first task event immediately and process full task in background', async () => {
    ...
    assert.isTrue(
      saveSpy.calledOnce,
      'Save should be called for the initial task creation',
    );
    ...
    assert.isTrue(
      saveSpy.calledTwice,
      'Save should be called twice (submitted and completed)',
    );
  ...
  it('sendMessage: should return second task with full history if message is sent to an existing, non-terminal task', async () => {
      ...
      async (ctx, bus) => {
        ...
        bus.publish({
          ...
          artifact: {
            ...
            description: 'A test artifact.',
            parts: [
              { kind: 'text', text: 'This is the content of the artifact.' },
            ],
          },
        });
    ...
    assert.equal(
      (secondTask.history![1].parts[0] as any).text,
    ...
    assert.equal(
      (secondTask.history![3].parts[0] as any).text,
      'Response to message 2',
    );
    assert.equal(
      secondTask.artifacts![0].artifactId,
      'artifact-1',
      'Artifact should be the same',
    );
    assert.equal(
  ...
  it('sendMessage: should return second task with full history if message is sent to an existing, non-terminal task, in non-blocking mode', async () => {
    ...
    assert.equal(
      (finalTask.history![1].parts[0] as any).text,
      'Response to message 1',
    );
    assert.equal(
    ...
    assert.equal(
      (finalTask.history![3].parts[0] as any).text,
      'Response to message 2',
    );
    assert.equal(
      finalTask.artifacts![0].artifactId,
      'artifact-1',
      'Artifact should be the same',
    );
    assert.equal(
      finalTask.artifacts![0].name,
      'Test Document',
      'Artifact name should be the same',
    );
    assert.equal(
  ...
  it('resubscribe: should allow multiple clients to receive events for the same task', async () => {
    ...
    assert.isTrue(
      saveSpy.calledThrice,
      'TaskStore.save should be called 3 times',
    );
  ...
  it('listTaskPushNotificationConfigs: should return all configs for a task', async () => {
    ...
    const listResponse =
      await handler.listTaskPushNotificationConfigs(listParams);
  ...
  it('should send push notification when task update is received', async () => {
    ...
    const params: MessageSendParams = {
      message: {
        ...createTestMessage(
          'msg-push-1',
          'Work on task with push notification',
        ),
        contextId: contextId,
      },
      configuration: {
    ...
    assert.isTrue(
      (mockPushNotificationSender as MockPushNotificationSender).send
        .calledThrice,
    );
    ...
    const secondCallTask = (
      mockPushNotificationSender as MockPushNotificationSender
    ).send.secondCall.args[0] as Task;
  ...
  it('sendMessageStream: should send push notification when task update is received', async () => {
    ...
    const params: MessageSendParams = {
      message: {
        ...createTestMessage(
          'msg-push-stream-1',
          'Work on task with push notification via stream',
        ),
        contextId: contextId,
      },
      configuration: {
  ...
  it('Push Notification methods should throw error if task does not exist', async () => {
    ...
    for (const method of methodsToTest) {
      try {
        ...
        assert.fail(
          `Method ${method.name} should have thrown for non-existent task.`,
        );
      } catch (error: any) {
  ...
  it('Push Notification methods should throw error if pushNotifications are not supported', async () => {
    ...
    for (const method of methodsToTest) {
      try {
        await (handler as any)[method.name](method.params);
        assert.fail(
          `Method ${method.name} should have thrown for unsupported push notifications.`,
        );
      } catch (error: any) {
        expect(error).to.be.instanceOf(A2AError);
  ...
  it('cancelTask: should cancel a running task and notify listeners', async () => {
    ...
    assert.isDefined(
      createdTask,
      'Task creation event should have been received',
    );
  ...
  it('cancelTask: should fail when it fails to cancel a task', async () => {
    ...
    try {
      ...
      assert.isTrue(
        failingCancellableExecutor.cancelTaskSpy.calledOnceWith(
          taskId,
          sinon.match.any,
        ),
      );
  ...
  it('should pass a RequestContext with expected content to agentExecutor.execute', async () => {
    ...
    expect(capturedRequestContext?.taskId).to.equal(
      incomingTaskId,
      'taskId should match',
    );
    expect(capturedRequestContext?.contextId).to.equal(
      incomingContextId,
      'contextId should match',
    );
